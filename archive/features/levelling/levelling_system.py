      ...
      async with asqlite.connect(tv.sql_dir) as connector:
        async with connector.cursor() as cursor:
          async with ctx.typing():
            ...
            '''await cursor.execute("SELECT xp FROM main WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))
            xp = await cursor.fetchone()

            await cursor.execute("SELECT messages FROM main WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))
            messages = await cursor.fetchone()

            await cursor.execute("SELECT level FROM main WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))
            level = await cursor.fetchone()

            await cursor.execute("SELECT money FROM main WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))
            money = await cursor.fetchone()

            await cursor.execute("SELECT total_xp FROM main WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))
            total_xp = await cursor.fetchone()

            await cursor.execute(f"SELECT * FROM main WHERE guild_id = {ctx.guild.id} AND user_id = {member.id}")
            results = await cursor.fetchall()

            warns = 0
            for result in results:
                reason = result["warning_text"]
            ...
            money = int(money[0])
            warnings = int(warns)
            total_xp = int(total_xp[0])
            messages = int(messages[0])'''

            await cursor.execute("SELECT xp, messages, level, money, total_xp FROM users WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))

            # Using fetchone() to retrieve the first row returned by the query and map() to convert the values to integers
            xp, messages, level, money, total_xp = map(int, await cursor.fetchone())

            await cursor.execute("SELECT warn_text FROM warnings WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, member.id)) #HERE

            #Using list comprehension to count the number of warnings
            warnings = sum(1 for result in await cursor.fetchall() if result["warn_text"] is not None)

            level_formula = int(level*(level * 10))
            xp_till_next_level = int(level_formula - xp)

            file = open("stats_avatar.jpg", "w")

            while file:

              await member.display_avatar.save("stats_avatar.jpg")
              color_thief = ColorThief("stats_avatar.jpg")
            ...
            embed = discord.Embed(title = "Statistics", color = int(embed_hex_code, 0))
          ...
          return os.remove("stats_avatar.jpg")
        ...
        async with connector.cursor() as cursor:
          async with ctx.typing():
            ...
            with Image.open("avatar.png","r") as image:
              ...
              await cursor.execute("SELECT xp, level, total_xp FROM users WHERE user_id = ? AND guild_id = ?", (member.id, member.guild.id))
              results = await cursor.fetchone()
              xp, level, total_xp = map(int,results)
