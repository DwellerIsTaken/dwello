...
class TimeoutSuggestion(View):
    ...
    def __init__(self: Self, bot: Dwello, ctx: DwelloContext, member: discord.Member, reason: str , timeout: int = None):
...
class Warnings(BaseCog):
    ...
    async def warning(self: Self, ctx: DwelloContext):
        ...
        embed = discord.Embed(description=f"```{ctx.clean_prefix}warning [command_name]```", color = cs.WARNING_COLOR) # prefix?
    ...
    @commands.guild_only()
    async def warn(self: Self, ctx: DwelloContext, member: discord.Member, reason: Optional[str] = None) -> Optional[discord.Message]:
    ...
    @commands.bot_has_permissions(moderate_members = True)
    @commands.guild_only()
    async def warnings(self: Self, ctx: DwelloContext, member: discord.Member = None) -> Optional[discord.Message]:
        async with ctx.typing(ephemeral=True):
            async with self.bot.pool.acquire() as conn:
                conn: asyncpg.Connection
                async with conn.transaction():
                    ...
                    embed.set_author(name=f"{apostrophize(member.name)} warnings", icon_url=str(member.display_avatar))
                    ...
                    if warns == 0:
                        return await ctx.reply(embed=discord.Embed(description=f"{string} no warnings yet.", color=cs.RANDOM_COLOR), user_mistake=True) # kind of success, so embed can be used

                    warns_1 = 0
                    for result in range(warns):

                        if warns_1 > 4:
                            embed.add_field(name = "\u2800", value = f"This member has **{warns - warns_1}** more warnings.")
                            break
                        ...
                        embed.add_field(name = f"Warning:   {int(warns_1) + 1}", value = f"Reason: `{reason_list[int(warns_1)]}`\nDate: *{date}*", inline = False) # -1
                        warns_1 += 1
            ...
            if suggest:
                embed: discord.Embed = discord.Embed(title = "A lot of warnings", description = f"Would you like to time **{member}** out for 12 hours?", color = cs.WARNING_COLOR)
                if warns > 3:
                    if ctx.author.guild_permissions.moderate_members: # == True ?
                        return await ctx.send(embed=embed, view = TimeoutSuggestion(self.bot, ctx, member, "Too many warnings!"))

    @warning.command(name='remove', help="Removes selected warnings.", with_app_command = True)
    @commands.bot_has_permissions(moderate_members = True)
    @commands.has_permissions(moderate_members = True)
    @commands.guild_only()
    async def remove_warn(self: Self, ctx: DwelloContext, member: discord.Member, warning: str) -> Optional[discord.Message]:
        async with ctx.typing(ephemeral=True):
            async with self.bot.pool.acquire() as conn:
                conn: asyncpg.Connection
                async with conn.transaction():
                    ...
                    if member == ctx.author:
                        return await ctx.reply(embed= discord.Embed(title="Permission Denied.", description= "**Do. Not. Attempt.**", color=cs.WARNING_COLOR), user_mistake=True)
