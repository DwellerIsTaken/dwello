    ...
    async def unban(self, ctx: commands.Context, member_object: str) -> Union[discord.Message, discord.InteractionMessage, None]:
        async with ctx.typing(ephemeral=True):
            ...
            try:
                ...
                return await ctx.reply(embed=discord.Embed(description="The provided member is un-banned.", color=tv.color), ephemeral = True, mention_author=False)

            except commands.UserNotFound:
                return await ctx.reply("The provided member doesn't exist or isn't banned.", ephemeral = True, mention_author=True)
    ...
    async def nick(self, ctx: commands.Context, member: discord.Member, *, nickname: Optional[str] = None) -> Optional[discord.Message]:
        async with ctx.typing(ephemeral=True):
            ...
            if not nickname and not member.nick:
                return await ctx.reply(f"**{member}** has no nickname to remove.", ephemeral = True, mention_author=True)

            elif nickname and len(nickname) > 32:
                return await ctx.reply(f"Nickname is too long! ({len(nickname)}/32)", ephemeral = True, mention_author=True)
