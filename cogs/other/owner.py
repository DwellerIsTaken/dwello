...
class Owner(BaseCog):
    ...
    async def blacklist_group(self: Self, ctx: DwelloContext, user: Union[discord.User, int] = None, *, reason: str = None):
        if user:
    ...
    async def add(self: Self, ctx: DwelloContext, user: Union[discord.User, int], *, reason: str = None) -> discord.Message:
        ...
        try:
            async with self.bot.safe_connection() as conn: 
                await conn.execute(
                    "INSERT INTO blacklist(user_id, reason) VALUES($1, $2)", user_id, reason
                )
    ...
    async def display(self: Self, ctx: DwelloContext) -> discord.Message:
        ...
        if records:
            for record in records:
                embed.add_field(name=mk(self.bot.get_user(record['user_id']).name), value=record['reason'])
        else:
