...
from utils import BaseCog, is_discord_link, get_unix_timestamp
...
class NewsViewer(discord.ui.View):
    ...
    async def get_embed(self: Self, page: Page) -> discord.Embed:
        ...
        embed = discord.Embed(title=f"\N{NEWSPAPER} {fm_dt(time)} ({fm_dt(time, 'R')})", colour=cs.RANDOM_COLOR)
        embed.add_field(name=page.title, value=message.content)

        #author = self.bot.get_user(page.author_id)
        #if author:
        embed.set_footer(text=f"ID: {page.news_id} - Authored by {message.author.name}", icon_url=message.author.display_avatar.url)
    ...
    async def add(self: Self, ctx: DwelloContext, link: str, *, title: str):
        ...
        if success:
            async with self.bot.safe_connection() as conn: 
                await conn.execute(
                    "INSERT INTO news(title, message_id, channel_id) VALUES($1, $2, $3)", title, message_id, channel_id
                )
