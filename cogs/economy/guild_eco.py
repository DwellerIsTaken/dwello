...
from typing import Optional, Any, Union, Literal, Tuple, TYPE_CHECKING
...
class GuildEcoUtils:
    ...
    async def fetch_basic_job_data_by_job_name(self: Self, ctx: DwelloContext, name: str) -> Optional[Tuple[Optional[int], Optional[str]]]:
    ...
    async def server_job_create(self: Self, ctx: DwelloContext, name: str, salary: int, description: str) -> Optional[discord.Message]:
        async with self.bot.pool.acquire() as conn:
            conn: asyncpg.Connection
            async with conn.transaction():
                ...
                if job_count == 10:
                    return await ctx.reply("You *cannot* have more than 10 jobs on your server, not yet!") # UNLOCK WITH PAYMENT (?) BRUH
    ...
    async def jobs_display(self: Self, ctx: DwelloContext) -> Optional[discord.Message]:
        async with self.bot.pool.acquire() as conn:
            ...
            async with conn.transaction():
                ...
                job_embed: discord.Embed = discord.Embed(title="Joblist", description="Jobs currently available on this server", color = cs.RANDOM_COLOR)
                failure_embed: discord.Embed = discord.Embed(description="No jobs currently available on this server", color=cs.WARNING_COLOR)
    ...
    async def server_job_remove(self: Self, ctx: DwelloContext, member: Optional[discord.Member]) -> Optional[discord.Message]:
        async with self.bot.pool.acquire() as conn:
            conn: asyncpg.Connection
            async with conn.transaction():
    ...
    async def server_job_delete(self: Self, ctx: DwelloContext, name: Union[str, Literal['all']]) -> Optional[discord.Message]:
        async with self.bot.pool.acquire() as conn:
            conn: asyncpg.Connection
            async with conn.transaction():
...
class Guild_Economy(BaseCog):
    ...
    async def server(self: Self, ctx: DwelloContext) -> Optional[discord.Message]:
        ...
        embed = discord.Embed(title="Denied", description="```$server [subgroup name]```", color=cs.RANDOM_COLOR) # TURN THESE DESCRIPTIONS INTO POINTERS TO HELP COMMAND
        return await ctx.reply(embed=embed)
    ...
    async def job_create(self: Self, ctx: DwelloContext, name: str, salary: commands.Range[int, 2000, 20000], description: Optional[str]):
    ...
    async def job_set(self: Self, ctx: DwelloContext, name: str) -> Optional[discord.Message]:
        ...
        await self.bot.db.fetch_table_data("jobs")
        return await ctx.reply(embed=discord.Embed(description=f"The job is set to: **{name}**", color = cs.RANDOM_COLOR), mention_author=False)
